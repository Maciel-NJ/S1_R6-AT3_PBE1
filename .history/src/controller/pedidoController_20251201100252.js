 ...
 */    atualizarPedido: async (req, res) => {
   ...
   buscarPedidoPorId: async (req, res) => {
        try {
            const id = Number(req.query.idPedido);
            if (!id || !Number.isInteger(id)) {
                return res.status(400).json({ message: 'Informe um ID válido! :/' });
            }
            const resultado = await pedidoModel.buscarPedidoPeloId(id);
            res.status(200).json({ message: 'Resultado do pedido procurado:  :)', data: resultado });
        ...
        try {
            ...
            if (produtoSelecionado.length === 0) {
                ...
                if (resultado.affectedRows === 1) {
                    res.status(200).json({ message: 'Pedido excluído com sucesso!', data: resultado });
                }
                else {
                    throw new Error('ão foi possível excluir o pedido');
                }
