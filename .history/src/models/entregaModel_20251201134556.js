 ...
 */  atualizarEntrega: async (valorDistancia, valorPeso, acrescimo, desconto, taxaExtra, valorFinal, statusEntrega, pedidoId) => {
    const sql = `
      UPDATE entregas 
      SET valor_distancia = ?, valor_peso = ?, acrescimo = ?, desconto = ?, taxa_extra = ?, valor_final = ?, status_entrega = ?
      WHERE id_pedido = ?
    `;
    const values = [valorDistancia, valorPeso, acrescimo, desconto, taxaExtra, valorFinal, statu    sEntrega, pedidoId];
