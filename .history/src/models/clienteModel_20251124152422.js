...
criarCadastro: async (pNomeCompleto, pCpf, pTelefone, pEmail, pEndereco) => {
 ...
 buscarTodos: async () => {
    ...
    alterarClientes: async (pNomeCompleto, pCpf, pTelefone, pEmail, pEndereco,) => {
        const sql = 'UPDAT clientes SET nome_completo=?, cpf=?, telefone=?, email=?, endereco=? WHERE id_cliente = ?;';
        const values = [pNomeCompleto, pCpf, pTelefone, pEmail, pEndereco];
        const [rows] = await pool.query(sql, values);
        return rows;
