...
const pedidoModel = {
    insertPedido: async (pIdCliente, pDataPedido, pIdProduto, , pValorItem, ) => {
        const connection = await pool.getConnection();
        try {
            await connection.beginTransaction();
            //insert 1 - tabela pedidos
            const sqlPedido = `INSERT INTO pedidos (id_cliente_fk, valor_total, data_pedido) VALUES (?, ?, ?)`;
            const valuesPedido = [pIdCliente, pQuantidadeItem * pValorItem, pDataPedido, pValorBaseKg, pValorBaseKm, pPeso ];
